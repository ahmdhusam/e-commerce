...
export class CartService {
  ...
  async cart(ownerId: string, limit: number, skip: number): Promise<Product[]> {
  ...
  async add(ownerId: string, productId: string, quantity: number): Promise<void> {
    const product = await this.productService.productToCart(productId, quantity);

    let cartItem = await this.cartRepo.findOneBy({ ownerId, productId });

    if (!cartItem) cartItem = this.cartRepo.create({ ownerId, productId, quantity });
  ...
  async delete(ownerId: string, productId: string): Promise<void> {
    ...
    await this.entityManager.transaction(async transactionManager => {
      await Promise.all([transactionManager.remove(cartItem), transactionManager.save(product)]);
    });
  }
